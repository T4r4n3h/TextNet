...
function ClientInputArea() {
  ...
  const submitWords = async (e) => {
    ...
    try {
      const data = { outWords };
      console.log(`i want to see what is in the data ${data}`);
      const response = await fetch("http://localhost:3007/api/words", {
        method: "POST",
        headers: { "content-Type": "application/json" },
        body: JSON.stringify(data),
      });
  ...
  return (
    <div className="mx-auto mt-10 border ">
      <div className="mx-auto border shadow-2xl w-25">
        <form onSubmit={onSubmitForm}>
          ...
          <div className="w-full flex justify-between">
            <button className="border-2   w-20  align-center bg-blue-400 text-white mb-4">
              Submit
            </button>
            <button className="border-2   w-20  align-center bg-blue-400 text-white mb-4 ">
              New Text
      ...
      <p>this will be the out put</p>
      <br />
      <div className="mx-auto flex flex-wrap ">
        {outWords.map((word) => (
          <Word word={word} key={word.id} />
        ))}
        <button onClick={submitWords} className="ml-auto ">
          Submit
        </button>
      </div>
    </div>
